sawblade_trap = class({})
LinkLuaModifier( "modifier_sawblade_trap_lua", LUA_MODIFIER_MOTION_NONE )

--------------------------------------------------------------------------------

function sawblade_trap:CastFilterResultTarget( hTarget )
	--print("Cast Filter")
	if IsServer() then
		--print("Is Server")
...
function sawblade_trap:GetCastRange( vLocation, hTarget )
...
function sawblade_trap:OnSpellStart()
	...
	if hTarget ~= nil then
		...
		if ( not hTarget:TriggerSpellAbsorb( self ) ) then
			...
			hTarget:AddNewModifier( self:GetCaster(), self, "modifier_sawblade_trap_lua", { duration = damage_delay } )
