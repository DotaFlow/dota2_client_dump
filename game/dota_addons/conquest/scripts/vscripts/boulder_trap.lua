boulder_trap = class({})
LinkLuaModifier( "modifier_boulder_trap_lua", LUA_MODIFIER_MOTION_NONE )

--------------------------------------------------------------------------------

function boulder_trap:CastFilterResultTarget( hTarget )
	--print("Cast Filter")
	if IsServer() then
		--print("Is Server")
...
function boulder_trap:GetCastRange( vLocation, hTarget )
...
function boulder_trap:OnSpellStart()
	...
	if hTarget ~= nil then
		...
		if ( not hTarget:TriggerSpellAbsorb( self ) ) then
			...
			hTarget:AddNewModifier( self:GetCaster(), self, "modifier_boulder_trap_lua", { duration = damage_delay } )
