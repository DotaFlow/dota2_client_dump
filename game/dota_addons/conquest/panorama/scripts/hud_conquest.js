...
function UpdateSharedCPStyles()
{
	...
	SetContestable( "#CP1", true);//all_points_captured);
	SetContestable( "#CP2", true);//all_points_captured);
	SetContestable( "#CP3", true); //middle point is always contestable
	SetContestable( "#CP4", true);//all_points_captured);
	SetContestable( "#CP5", true);//all_points_captured);
...
//  $.Msg( "OnCountdownTimer: ", timerText );
...
{
//	$.Msg( "OnNetTableChanged: ", table, ", ", key, ", ", data );
	UpdateCPNew( data );
}
...
{
//	$.Msg( "DisplayTeamPoints: ", data );
	var radiant_points = data.radiant;
	var dire_points = data.dire;
//	$( "#Radiant" ).text = radiant_points;
//	$( "#Dire" ).text = dire_points;
...
(function () {
	...
	// set up the UI for each control point
    for ( var cp of CONTROL_POINT_INFO )
    {
...
//		cpRootNode.FindChildInLayoutFile( "label" ).text = $.Localize( cp.label_text );
//		$.Msg( "BLoadLayout" );
    }

    for ( var cp_initial_state of CustomNetTables.GetAllTableValues( "control_points" ) )
    {
//		$.Msg( "cp_initial_state: ", cp_initial_state.value );
